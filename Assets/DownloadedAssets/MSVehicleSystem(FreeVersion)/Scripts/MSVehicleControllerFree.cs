...
public class MSVehicleControllerFree : MonoBehaviour
{
	...
	void Update()
	{
		...
		if (!changinGearsAuto)
		{
			engineInput = Mathf.Clamp01(inputs.VerticalMovementInput);
		}
		else
		{
			engineInput = 0;
	...
	void FixedUpdate()
	{
		...
		if (wheelFDIsGrounded && wheelFEIsGrounded && wheelTDIsGrounded && wheelTEIsGrounded)
		{
			...
			{
				absBrakeInput = Mathf.Abs(Mathf.Clamp(inputs.VerticalMovementInput, -1.0f, 0.0f));
			}
			else if (currentGear <= 0 && mediumRPM < 0)
			{
				absBrakeInput = Mathf.Abs(Mathf.Clamp(inputs.VerticalMovementInput, 0.0f, 1.0f)) * -1;
			}
			else
			{
				absBrakeInput = 0.0f;
	...
	void SetWheelForces(WheelCollider wheelCollider)
	{
		...
		if (wheelCollider.isGrounded)
		{
			...
			if (Mathf.Abs(inputs.HorizontalMovementInput) > 0.1f && wheelCollider.steerAngle != 0.0f && Mathf.Sign(wheelCollider.steerAngle) != Mathf.Sign(tireSL.x))
			{
	...
	{
		wheelCollider.GetGroundHit(out tempWheelHit);
		return (localVelocityVector.y + (tempWheelHit.sidewaysSlip * ((Mathf.Abs(inputs.VerticalMovementInput) + Mathf.Abs(inputs.HorizontalMovementInput)) / 2.0f) * (-2.0f))) / wheelCollider.radius;
	}
	...
	public void TireSlips(WheelCollider wheelCollider, WheelHit wheelHit)
	{
		...
		if (wheelCollider.isGrounded)
		{
			...
			{
				minSlipYTireSlips = Mathf.Min(Mathf.Abs(reverseForce * tireSlipTireSlips.x) / downForceTireSlips, Mathf.Clamp((inputs.VerticalMovementInput * 2.5f), -2.5f, 1.0f));
	...
	{
		if (Mathf.Abs(inputs.HorizontalMovementInput) < 0.9f)
		{
	...
	void StabilizeAirRotation()
	{
		if (!colliding)
		{
			...
			if (!wheelFDIsGrounded && !wheelFEIsGrounded && !wheelTDIsGrounded && !wheelTEIsGrounded && !isGroundedExtraW)
			{
				...
				if (Mathf.Abs(inputs.HorizontalMovementInput) > 0.1f)
				{
					ms_Rigidbody.AddTorque(transform.forward * -inputs.HorizontalMovementInput * _vehicleSettings.vehicleMass * 0.6f);
				}
				if (Mathf.Abs(inputs.VerticalMovementInput) > 0.1f)
				{
					ms_Rigidbody.AddTorque(transform.right * inputs.VerticalMovementInput * _vehicleSettings.vehicleMass * 0.44f);
				}
			}
		}
	}
	...
	void StabilizeWheelRPM()
	{
		if (currentGear > 0)
		{
			if (KMh > (_vehicleTorque.maxVelocityGears[currentGear - 1] * _vehicleTorque.speedOfGear) && Mathf.Abs(inputs.VerticalMovementInput) < 0.5f)
			{
		...
		{
			if (KMh > (_vehicleTorque.maxVelocityGears[0] * _vehicleTorque.speedOfGear) && Mathf.Abs(inputs.VerticalMovementInput) < 0.5f)
			{
				if (_wheels.rightFrontWheel.wheelDrive)
				{
	...
	{//aqui
		...
		if (mediumRPM < -0.1f && Mathf.Abs(inputs.VerticalMovementInput) < 0.1f)
		{
			currentGear = -1;
		}
		if (Mathf.Abs(inputs.VerticalMovementInput) < 0.1f && mediumRPM >= 0 && currentGear < 2)
		{
			currentGear = 1;
		}

		if ((Mathf.Abs(Mathf.Clamp(inputs.VerticalMovementInput, -1f, 0f))) > 0.8f)
		{
		...
		if ((Mathf.Abs(Mathf.Clamp(inputs.VerticalMovementInput, 0f, 1f))) > 0.8f)
		{
		...
		if (currentGear > 0)
		{
			...
			if (inputs.VerticalMovementInput > 0.1f && KMh > (_vehicleTorque.idealVelocityGears[currentGear - 1] * _vehicleTorque.speedOfGear + 7 * _vehicleTorque.speedOfGear))
			{
	...
	void Volant()
	{
		angle1Ref = Mathf.MoveTowards(angle1Ref, inputs.HorizontalMovementInput, 2 * Time.deltaTime);
		angle2Volant = Mathf.MoveTowards(angle2Volant, inputs.HorizontalMovementInput, 2 * Time.deltaTime);
	...
	public float VehicleTorque(WheelCollider wheelCollider)
	{
		...
		if ((Mathf.Abs(inputs.VerticalMovementInput) < 0.5f) || KMh > _vehicleTorque.maxVelocityKMh)
		{
			return 0;
		}
		...
		if (currentGear < 0)
		{
			clampInputTorque = Mathf.Abs(Mathf.Clamp(inputs.VerticalMovementInput, -1f, 0f));
	...
	void ApplyTorque()
	{
		leftDifferential = 1 + Mathf.Abs((0.2f * Mathf.Abs(Mathf.Clamp(inputs.HorizontalMovementInput, 0, 1))) * (angleRefVolant / 60));
		rightDifferential = 1 + Mathf.Abs((0.2f * Mathf.Abs(Mathf.Clamp(inputs.HorizontalMovementInput, -1, 0))) * (angleRefVolant / 60));
		//torque do motor
		if (_wheels.rightFrontWheel.wheelDrive)
		{
	...
	void Brakes()
	{
		...
		brakeVerticalInput = inputs.VerticalMovementInput;
	...
	void ApplyBrakeInWheels(WheelCollider wheelCollider, bool handBrake)
	{
		...
		if (!wheelCollider.isGrounded && Mathf.Abs(wheelCollider.rpm) > 0.5f && Mathf.Abs(inputs.VerticalMovementInput) < 0.05f && wheelCollider.motorTorque < 5.0f)
		{
			wheelCollider.brakeTorque += _vehicleSettings.vehicleMass * Time.deltaTime * 50;
		}
		if (KMh < 0.5f && Mathf.Abs(inputs.VerticalMovementInput) < 0.05f)
		{
